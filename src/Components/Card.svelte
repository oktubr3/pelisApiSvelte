...
<div class="relative overflow-hidden text-white transition duration-500 ease-in-out transform shadow-lg rounded-xl hover:-translate-y-2 hover:shadow-2xl movie-item movie-card" data-movie-id={id}>
    <div class="mt-52 absolute inset-0 z-10 transition duration-300 ease-in-out bg-gradient-to-t from-black via-gray-900"></div>
    <div class="relative z-10 px-10 pt-10 space-y-6 cursor-pointer group movie_info">
    ...
    <img class="absolute inset-0 w-full transform -translate-y-4" src="http://image.tmdb.org/t/p/w500{image}" alt={title} style="filter: grayscale(0); " />
    <div class="relative z-10 flex flex-row pb-10 space-x-4 poster__footer">
        <button class="flex items-center mx-auto py-2 transition bg-red-500 border-0 rounded-md -white ftemont-medium itext-sm px-4 focus:outline-none focus:ring hover:bg-red-600 active:bg-red-700 focus:ring-red-300" type="submit" on:click={ver}>Mas Info...</button>
    </div>
